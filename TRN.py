...
class TRN(nn.Module):
    def __init__(self, batch_size, channel, width, height,
        ...
        self.g_0 = nn.Sequential(
                nn.Linear(2*(channel+2)+self.tv_size, 256),
                nn.ReLU(),
                nn.Linear(256, 256),
                nn.ReLU(),
                nn.Linear(256, 256),
    ...
    def forward(self, cnn_tensor):
        ...
        # (b,tv_size) -> (b,1,tv_size) -> (b,w*h,w*h,tv_size)
        cnn_total = torch.unsqueeze(cnn_total, 1)
        cnn_total = cnn_total.repeat(1,w*h,1)
        cnn_total = torch.unsqueeze(cnn_total, 2)
        cnn_total = cnn_total.repeat(1,1,w*h,1)
        
        # Concatenate cnn dot & cnn total
        # (b,w*h,w*h,2(c+2)+tv_size)
        cnn_full = torch.cat([cnn_dot_i, cnn_dot_j, cnn_total], 3)
        
        # (b,w*h,w*h,2(c+2)+tv_size) -> (b*w*h*w*h,2(c+2)+tv_size)
        cnn_full = cnn_full.view(batch_size*w*h*w*h,2*(c+2)+tv_size)
        
        if self.method == 0:
            # g_0
            # (b*w*h*w*h,2(c+2)+tv_size) -> (b,256)
            ...
            # g_1
            # (b*w*h*w*h,2(c+2)+tv_size) -> (b,w*h*w*h*2)
